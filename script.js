...
saveBtn.onclick = () => {
    ...
    a.download = 'studio_master_v6.3.mp3';
    a.click();
};

// Effect Application Wrapper
...
document.getElementById('applyEqBtn').onclick = () => {
    const bands = frequencies.map((f, i) => ({ f, g: bandGains[i] }));
    applyEffect(AudioEffects.applyEQ, bands);
};

document.getElementById('exciterBtn').onclick = () => applyEffect(AudioEffects.applyExciter);
document.getElementById('deesserBtn').onclick = () => applyEffect(AudioEffects.applyDeEsser);
document.getElementById('stereoBtn').onclick = async () => {
    ...
    try {
        ...
        const a = document.createElement('a');
        a.href = currentObjectURL;
        a.download = 'studio_stereo_v6.3.mp3';
        a.click();
...
document.getElementById('compressorBtn').onclick = () => applyEffect(AudioEffects.applyCompressor);
document.getElementById('reverbBtn').onclick = () => applyEffect(AudioEffects.applyReverb);
document.getElementById('delayBtn').onclick = () => applyEffect(AudioEffects.applyDelay);
...
document.getElementById('noiseRedBtn').onclick = () => applyEffect(async (buf) => {
    ...
    for(let i=0; i<buf.length; i++) out[i] = Math.abs(buf[i]) < threshold ? 0 : buf[i];
    return out;
});
document.getElementById('distortionBtn').onclick = () => applyEffect(AudioEffects.applyDistortion);
document.getElementById('fadeInBtn').onclick = () => applyEffect(AudioEffects.fadeIn, 2);
document.getElementById('fadeOutBtn').onclick = () => applyEffect(AudioEffects.fadeOut, 2);
